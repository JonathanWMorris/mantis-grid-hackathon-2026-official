...
# Development convenience only. Judging supplies environment variables and the
# Docker build excludes .env. Never override the evaluator's key or endpoint.
...
def main() -> None:
    ...
    p.add_argument("--agent", default="agents.magentic",
    ...
    pending = queries[~queries.row_id.astype(int).isin(done)]
    for position, r in enumerate(pending.itertuples(index=False)):
        rid = int(r.row_id)
        if rid in done:
            continue
        t0 = time.time()
        ctx.update(row_id=rid, remaining_cases=len(pending)-position,
    ...
    summary = {'cases': len(rows), 'wall_seconds': time.monotonic() - run_started,
    ...
    if cgroup_peak.exists():
        summary['peak_container_memory_bytes'] = int(cgroup_peak.read_text())
